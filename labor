...
int main (int argc, char **argv){
	...
	if (fscanf(file, "%d %d", &n, &m) != 2){
		printf ("Убедитесь, что файл содержит 2 числа в первой строке (размеры матрицы)\n");
		fclose(file);
		return 1;
	}
    ...
    for (int i = 0; i<n; i++){
        ...
        for (int j = 0; j<m; j++){
            if (fscanf(file, "%d", &matrix[i][j]) != 1) {
                printf("Ошибка чтения элемента matrix[%d][%d] из файла. Убедитесь, что файл содержит достаточное количество чисел.\n", i, j);
    ...
    if (fscanf(file, "%d", &temp) != EOF) {
        printf("В файле остались непрочитанные данные\n");
    }
